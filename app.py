...
def stats():
    ...
    with cursor() as cur:
        ...
        # online: last_active within 5 minutes (if column exists)
        try:
...
# You can add your search handlers (process_search) here mirroring earlier code.
...
if __name__ == "__main__":
    ...
    # polling must be on its own line (fixed)
    bot.polling(none_stop=True, timeout=60)    created=[]
    with cursor() as cur:
        for _ in range(count):
            for attempt in range(10):
                k = f"KEY-{random.randint(100000,999999)}"
